...
function mockTemplate() {
    ...
    var options = '<place_options_here>';
    ...
    newModule.config(['$provide', '$httpProvider', function($provide, $httpProvider){
        ...
        function equals (property, expectationRequest, config) {
            var expected = expectationRequest[property];
            var actual = config[property];
            ...
            if (actual === expected) {
                ...
                for (var key in expected) {
                    good = good && equals(key, expected, actual);
                }
                return good;
            }
        }

        function matchLooseProperty(property, expectationRequest, config){
            return !expectationRequest[property] || equals(property, expectationRequest, config);
        }

        function matchParams (expectationRequest, config) {
            if (options.paramsLooseMatching) {
                return matchLooseProperty('params', expectationRequest, config);
            } else {
...
module.exports = function (expectations, plugins, options) {
    ...
    var newFunc =
        template
            ...
            .replace(/'<place_options_here>'/, JSON.stringify(options || {}));
