...
function getConfig(){
	...
	if(module.exports.config){
		...
		config.options = module.exports.config.options || config.options;
	}
	...
	config.options = protractorConfig.options || defaultConfig.options;
	return config;
}
...
module.exports = function(mocks, plugins, skipDefaults, options){
	var builtMocks = buildMocks(mocks, skipDefaults),
		builtPlugins = buildPlugins(plugins),
		builtOptions = buildOptions(options, skipDefaults);

	var ptor = getProtractorInstance();
	ptor.addMockModule('httpMock', httpMock(builtMocks, builtPlugins, builtOptions));
};
